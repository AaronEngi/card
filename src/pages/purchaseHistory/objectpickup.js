...
import Objectpickupitem from './objectpickupitem'
import IconDown from './images/price_down_arrow.png';
import IconUp from './images/price_up_arrow.png';
import Arrow_down from './images/arrow_down.png';

export default class Objectpickup extends React.Component {
  state = {
    items:[], 
    loading: true,
    tab:0,
  ...
  render() {
    ...
    var repoList = lists.map(function (repo, index) {
        return (
               <Objectpickupitem silverprice={repo.silverprice} purchaseDate={repo.purchaseDate} 
                          orderID={repo.orderID} orderState={repo.orderState} 
                          amountIncome={repo.amountIncome} purchaseWay={repo.purchaseWay}
                          purchaseWeight={repo.purchaseWeight} address={repo.address}></Objectpickupitem>
               );
            });
    }
    return (
		<InvestPage title="实物提取记录">
      {repoList}
		</InvestPage>
    );
  }
